...
- **ソースコード内ドキュメント（RustDoc）の更新**:
  コードの機能追加・仕様変更・リファクタリングなどを行う際は、必ず対応する RustDoc コメント（`///` や `//!`）を追記・修正し、`cargo doc` を実行して最新のドキュメントが正常にビルドできることを確認すること。また、ドキュメント内に記述した使用例（Doc-tests）の動作も `cargo test` で検証すること。
